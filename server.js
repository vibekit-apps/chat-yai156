...
const TYPES = {'.html':'text/html; charset=utf-8','.js':'application/javascript; charset=utf-8','.webmanifest':'application/manifest+json; charset=utf-8','.json':'application/json; charset=utf-8','.png':'image/png','.svg':'image/svg+xml'};
http.createServer((req, res) => {
  try {
    const safePath = req.url === '/' ? 'index.html' : decodeURIComponent(req.url).split('?')[0].replace(/^\/+/, '');
    const file = path.resolve(__dirname, safePath);
    if (!file.startsWith(__dirname) || !fs.existsSync(file) || fs.statSync(file).isDirectory()) throw new Error('not found');
